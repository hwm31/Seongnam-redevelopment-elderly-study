...
# 2) 공공편의시설 만족도 평균 계산 및 불필요한 열 삭제
new_social23['수원도서관이용만족도코드1_1'] = new_social23[['수원도서관이용만족도코드1_1', '수원도시공원이용만족도코드1_1']].mean(axis=1)
new_social23 = new_social23.drop(columns=['수원도시공원이용만족도코드1_1'])
...
new_social23.columns = [
    ...
    '공공편의시설만족도', # 기존 '수원도서관이용만족도코드1_1' (두 개 평균)
    '의료시설만족도'      # 기존 '수원보건소이용만족도코드1_1'
]
...
# 2) 공공편의시설 만족도 평균 계산 및 불필요한 열 삭제
new_social19['45-2.도서관_이용만족도_수원'] = new_social19[
    ['45-2.도서관_이용만족도_수원', '47-2.도시공원_이용만족도_수원']
].mean(axis=1)
new_social19 = new_social19.drop(columns=['47-2.도시공원_이용만족도_수원'])
# 3) 의료기관 만족도 평균 계산 및 불필요한 열 삭제
new_social19['49-2.의료기관_만족도_수원'] = new_social19[
    ['49-2.의료기관_만족도_수원', '50-1.보건소_만족도_수원']
].mean(axis=1)
new_social19 = new_social19.drop(columns=['50-1.보건소_만족도_수원'])
# 3) 최종 컬럼 이름(총 13개) 한 번에 지정
...
# 2) 공공편의시설(도서관·공원) 만족도 평균
new_social17['42-2.도서관만족도'] = new_social17[
    ['42-2.도서관만족도',
     '47-2.도시공원이용만족도']
].mean(axis=1)
new_social17 = new_social17.drop(columns=['47-2.도시공원이용만족도'])

# 3) 의료시설(의료기관·보건소) 만족도 평균
new_social17['44-2.이용의료시설만족도'] = new_social17[
    ['44-2.이용의료시설만족도',
     '45-1.보건시설이용만족도']
].mean(axis=1)
new_social17 = new_social17.drop(columns=['45-1.보건시설이용만족도'])
...
"""
temp.loc[temp['주차장이용만족도코드1_1'] == 6, '주차장이용만족도코드1_1'] = pd.NA
temp = temp.astype('float')
new_social23['주택만족도코드1_1'] = temp.mean(axis=1).round()
new_social23 = new_social23.drop(columns=['기반시설만족도코드1_1','주차장이용만족도코드1_1'])
new_social23.columns =  ['만나이','지역거주기간','향후거주의향','정주의식','거주지소속감','거주지만족도','월평균가구소득','부채유무','삶의만족도']
mapping = {1: 4, 2: 3, 3: 2, 4: 1}
new_social23['거주지소속감'] = new_social23['거주지소속감'].map(mapping)
new_social23['삶의만족도'] = new_social23['삶의만족도'].map({0:1, 1:1, 2:4, 3:4, 4:3, 5:3, 6:3, 7:2, 8:2, 9:1, 10:1})
"""
